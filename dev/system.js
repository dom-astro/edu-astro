...
function initSystem(exoPlanets, star) {

  dropSVG();
  var planets = exoPlanets.filter(planet => planet["Etoile"] == star && planet["Statut"] == "Confirmed");
...
function initOrbit(planets) {
  ...
  var orbits = orbitsContainer.selectAll("g.orbit")
							  ...
							  .attr("ry", function(d) {return d.major * Math.sqrt(1-d.excentricite*d.excentricite);})
							  .style("fill", "#3E5968")
...
function coefZH(star) {
	var tSun=5750;
	var tStar=star["Température de l'étoile"];
	var rStar=star.star_rayon;
